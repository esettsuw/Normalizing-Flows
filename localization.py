...
def export_gradient_maps(model, testloader, optimizer, n_batches=1):
    ...
    for i, data in enumerate(tqdm(testloader, disable=c.hide_tqdm_bar)):
        ...
        if c.pro_score:
            mask = inputs_unnormed[:,:,:,0].copy()
            mask[mask < 0.5] = 0
            mask[mask >= 0.5] = 1
            list_inputs_unnormed.append(mask.astype(int))
            list_grad_img_sq.append(grad_img_sq)
        ...
        if i == n_batches:
            break
    if c.pro_score:
        mask_np = np.vstack(list_inputs_unnormed).flatten()
        loc_np = np.vstack(list_grad_img_sq).flatten()
